import { MOCK_WEBHOOKS, WebhookId } from "@/constants/data";
...
export const WebhookTab = ({ tournamentId }: { tournamentId: string }) => {
  ...
  const selectedWebhook = useMemo(
    () => MOCK_WEBHOOKS.find((w) => w.id === selectedId)!,
    [selectedId],
  );
        ...
        <aside className="lg:col-span-4 space-y-6">
          <div className="space-y-3">
            ...
            {MOCK_WEBHOOKS.map((w) => (
              <button
                key={w.id}
                onClick={() => setSelectedId(w.id as WebhookId)}
                className={`w-full text-left p-5 border-2 transition-all group
                        ${selectedId === w.id ? "border-indigo-500 bg-[#111]" : "border-zinc-800 bg-zinc-900/40 opacity-60 hover:opacity-100"}`}
              >
                <h4 className="font-black italic text-sm">{w.name}</h4>
                <p className="text-[8px] mt-1 text-zinc-500">{w.description}</p>
              </button>
            ))}
          </div>
        </aside>
