...
export async function getTournamentRoleRequestsAction(tournamentId: string) {
  return await safeExecute(async () => {
    ...
    if (!participant) {
      throw new ForbiddenError({
        message:
          "Você não tem permissão para acessar a lista de solicitações de cargo",
      });
    }

    return await TournamentOrchestrator.feed.getTournamentRoleRequests(
      tournamentId,
    );
  });
}
